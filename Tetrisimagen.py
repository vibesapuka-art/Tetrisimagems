...
# --- INTERFACE ---
st.set_page_config(page_title="Bazzott Pro Studio", layout="wide")
...
with st.sidebar:
    ...
    if st.button("✅ Aplicar em Todos", width='stretch'):
...
if st.session_state.galeria:
    ...
    for i, item in enumerate(st.session_state.galeria):
        ...
        with st.expander(f"🖼️ {item['name']}", expanded=True):
            ...
            with c1:
                st.image(item['img'], width=100) # Mantido width simples para evitar erro de log [cite: 16]
            
            with c2:
                # Inicialização segura para o Massa funcionar
                ...
                t = st.number_input("Tamanho (cm)", 1.0, 25.0, key=f"t_{iid}")
                q = st.number_input("Qtd", 1, 500, key=f"q_{iid}")
    ...
    if st.button(f"🚀 GERAR PDF ({len(pecas_preparadas)} FIGURAS)", width='stretch'):
        with st.spinner("Montando páginas..."):
            folhas = montar_folhas(pecas_preparadas, margem)
            if folhas:
                for idx, folha in enumerate(folhas):
                    st.image(folha, caption=f"Página {idx+1}", width='stretch') # Atualizado para width='stretch' [cite: 17]
                
                buf = io.BytesIO()
                folhas[0].save(buf, format="PDF", save_all=True, append_images=folhas[1:], resolution=300.0)
                st.download_button("📥 BAIXAR PDF", buf.getvalue(), "Bazzott_Final.pdf", width='stretch')
