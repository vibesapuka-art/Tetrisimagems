...
def montar_folha_pro(lista_config, margem_mm, sangria_mm, espaco_mm, suavidade, linha_ativa, linha_px):
    ...
    for item in lista_config:
        ...
        # --- NOVO: Adiciona a imagem repetidas vezes na lista de processamento ---
...
st.title("✂️ Organizador de Topos: Com Repetição de Peças")

uploads = st.file_uploader("Suba seus personagens PNG", type=['png'], accept_multiple_files=True)

if uploads:
    config_list = []
    cols = st.columns(4) # Ajustado para caber os controles
    for i, arq in enumerate(uploads):
        with cols[i % 4]:
            ...
            # NOVO: Campo para escolher a quantidade de cópias
            qtd = st.number_input(f"Cópias", 1, 100, 1, key=f"q_{i}")
            config_list.append({'img': img_aberta, 'width_mm': larg, 'quantidade': qtd})

    if st.button("🚀 GERAR MONTAGEM"):
        with st.spinner('Organizando todas as cópias na folha...'):
            ...
            st.download_button("📥 Baixar PDF com Cópias", buf.getvalue(), "folha_com_copias.pdf")
