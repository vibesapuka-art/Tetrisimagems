...
with st.sidebar:
    ...
    if st.button("✅ Aplicar Todos", key="mass_apply"):
...
if u:
    for f in u:
        # ID Único para evitar duplicatas nos logs 
...
if st.session_state.galeria:
    ...
    for i, item in enumerate(st.session_state.galeria):
        ...
        with st.expander(f"📦 {item['name']}", expanded=True):
            ...
            with c2:
                t = st.number_input("cm", 1.0, 25.0, key=f"t_{iid}", value=st.session_state.get(f"t_{iid}", 4.0))
                q = st.number_input("un", 1, 500, key=f"q_{iid}", value=st.session_state.get(f"q_{iid}", 1))
            with c3:
            ...
            with c4:
                if st.button("❌", key=f"del_{iid}"): remover_idx = i

            res = gerar_contorno_individual(item['img'], t, s, l, suave)
            for _ in range(int(q)): pecas_preparadas.append(res)

    if remover_idx != -1:
        st.session_state.galeria.pop(remover_idx)
        st.rerun()

    if st.button(f"🚀 GERAR PDF ({len(pecas_preparadas)} ITENS)", use_container_width=True):
        ...
        st.download_button("📥 BAIXAR PDF", buf.getvalue(), "Bazzott_Final.pdf", use_container_width=True)
