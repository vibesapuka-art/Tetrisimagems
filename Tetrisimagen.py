...
def gerar_contorno_individual(img, medida_cm, sangria_cm, linha_ativa, nivel_suavidade, espelhar):
    ...
    # NOVO: Espelhamento para bandeirinhas
    if espelhar:
...
if u:
    for f in u:
        # Gerar ID único para evitar o erro de ID duplicado dos logs
        id_f = f"{f.name}_{time.time()}_{len(st.session_state.galeria)}"
        st.session_state.galeria.append({"id": id_f, "name": f.name, "img": Image.open(f).copy()})
    st.rerun()
...
if st.session_state.galeria:
    ...
    for i, item in enumerate(st.session_state.galeria):
        ...
        with st.expander(f"Configurar: {item['name']}", expanded=True):
            ...
            with c_del:
                if st.button("❌", key=f"del_{fid}"): indices_remover.append(i)
            with c1: 
                st.image(item['img'], width=60)
                # Opção de espelhar para fazer o verso
                esp = st.checkbox("Espelhar", key=f"esp_{fid}")
            with c2:
                t = st.number_input("cm", 1.0, 25.0, key=f"m{fid}", value=4.0)
                q = st.number_input("un", 1, 500, key=f"q{fid}", value=10)
            with c3:
                s = st.slider("Sangria", 0.0, 1.0, key=f"s{fid}", value=0.25, step=0.05)
                l = st.checkbox("Corte", True, key=f"l{fid}")
            
            p = gerar_contorno_individual(item['img'], t, s, l, suave, esp)
            for _ in range(q): pecas_preparadas.append(p)
    ...
    if st.button(f"🚀 GERAR PDF ({len(pecas_preparadas)} figuras)", use_container_width=True):
        ...
        if folhas:
            ...
            st.download_button("📥 BAIXAR PDF FINAL", pdf_bytes.getvalue(), "Bazzott_Editor.pdf", use_container_width=True)
